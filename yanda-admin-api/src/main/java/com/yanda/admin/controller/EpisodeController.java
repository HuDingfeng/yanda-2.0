...
public class EpisodeController extends BaseController {
	...
	@RequestMapping(value = "/getEpisodelist", method = RequestMethod.GET)
	public JsonResult getEpisodelist(HttpServletRequest request) {
		String pageNum = getValue(request, "pageNum", "1");
		String pageSize = getValue(request, "pageSize", "10");
		String searchVal = getNotEmptyValue(request, "searchVal");

		PageResult<EpisodeInfo> episodes = episodeService.getList(Integer.valueOf(pageNum), Integer.valueOf(pageSize), searchVal);
